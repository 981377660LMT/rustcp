...
#[derive(Clone, Debug)]
pub struct Row<S: Monoid + Add<U, Output = S>, U: Monoid> {
    pub left: usize,
    pub right: usize,
    pub father: usize,
    pub reverse: bool,
...
impl<S: Monoid + Add<U, Output = S>, U: Monoid> Row<S, U> {
    pub fn new(s: S, u: U, tree_weight: usize, id: usize) -> Self {
        Self {
            left: 0,
            right: 0,
            father: 0,
...
/// ```
///
pub struct LinkCutTree<S: Monoid + Add<U, Output = S>, U: Monoid> {
    nodes: Vec<Row<S, U>>,
}

impl<S: Monoid + Add<U, Output = S>, U: Monoid> Debug for LinkCutTree<S, U> {
...
impl<S: Monoid + Add<U, Output = S>, U: Monoid> LinkCutTree<S, U> {
    pub fn new(n: usize, tree_weight: impl Fn(usize) -> usize) -> Self {
        let mut res = Self {
            nodes: Vec::with_capacity(n + 1),
        };
        res.nodes.push(Row::new(S::zero(), U::zero(), 0, 0));
        for i in 0..n {
            res.nodes
                .push(Row::new(S::zero(), U::zero(), tree_weight(i), i + 1));
        }
    ...
    pub fn init(&mut self, mut id: usize) {
        self.nodes[id + 1] = Row::new(S::zero(), U::zero(), self.nodes[id + 1].tree_weight, id);
    }
    ...
    fn reverse_internal(&mut self, id: usize) {
        ...
        let tmp = self.nodes[id].sum;
        self.nodes[id].sum = self.nodes[id].rev_sum;
        self.nodes[id].rev_sum = tmp;
    }
    ...
    fn push_up(&mut self, id: usize) {
        ...
        self.nodes[id].rev_sum =
            self.nodes[right].rev_sum + self.nodes[id].weight + self.nodes[left].rev_sum;
    }
    ...
    pub fn set(&mut self, id: usize, t: S) {
        self.set_internal(id + 1, t);
    }

    fn set_internal(&mut self, id: usize, t: S) {
    ...
    fn modify_internal(&mut self, id: usize, u: U) {
        ...
        should_eq!(self.nodes[id].father, NIL);
        self.nodes[id].upd = self.nodes[id].upd + u;
        self.nodes[id].weight = self.nodes[id].weight + u;
        self.nodes[id].sum = self.nodes[id].sum + u;
    }
    ...
    pub fn node<'a>(&'a self, x: usize) -> &'a Row<S, U> {
        &self.nodes[x + 1]
    }

    pub fn node_mut<'a>(&'a mut self, x: usize) -> &'a mut Row<S, U> {
        &mut self.nodes[x + 1]
    }
    ...
    fn to_string_tree_structure_internal(
        &mut self,
        root: usize,
        fa: usize,
        consumer: &mut impl FnMut(usize, usize),
    ) {
        if root == NIL {
            return;
        }
    ...
    pub fn to_string_tree_structure(&mut self) -> Vec<Vec<usize>> {
        ...
        for i in 1..res.len() + 1 {
            if self.nodes[i].father == NIL {
                self.to_string_tree_structure_internal(
                    i,
                    self.nodes[i].tree_father,
                    &mut |fa, child| {
                        if fa < child {
...
impl<S: Monoid + Add<U, Output = S> + std::fmt::Display, U: Monoid> LinkCutTree<S, U> {
