use std::{cmp::{Ordering, max}, mem::swap};
...
/// ```
///
impl<T: Concrete + PreferDiv> LichaoSegtree<T> {
    ...
    fn insert_internal(
        ...
        if cover!(L, R, l, r) {
            ...
            if self.0[root].line.0 == line.0 {
                self.0[root].line.1 = max(self.0[root].line.1, line.1);
                return;
            }
    ...
    fn query_internal(&self, root: usize, x: usize, l: usize, r: usize) -> T {
        ...
        let m = (l + r) >> 1;
        if x <= m {
            max(best, self.query_internal(root + 1, x, l, m))
        } else {
            max(best, self.query_internal(self.0[root].right, x, m + 1, r))
        }
    }
}
