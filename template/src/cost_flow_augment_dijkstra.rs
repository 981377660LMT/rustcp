...
pub struct CostFlowAugmentDijkstra<'a, C: Number, E: CostFlowEdge<C>> {
    last_dist: Vec<C>,
    cur_dist: Vec<C>,
...
impl<'a, C: Number, E: CostFlowEdge<C>> CostFlowAugmentDijkstra<'a, C, E> {
    ...
    pub fn fix_dist(&mut self) {
        for i in 0..self.g.len() {
            self.last_dist[i] = unsafe_min(self.cur_dist[i] + self.last_dist[i], self.inf);
        }
    }
    ...
    pub fn try_push(&mut self, send: C) -> (C, C) {
        ...
        while remain > C::ZERO {
            ...
            while let Some((root, index)) = trace {
                ...
                max_flow = unsafe_min(max_flow, rev_edge(self.g, e).0.flow());
...
impl<'a, C: Number, E: CostFlowEdge<C>> CostFlowAugment<C> for CostFlowAugmentDijkstra<'a, C, E> {
