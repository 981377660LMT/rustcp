    ...
    T: Clone + Debug + Ord
{
    pub fn new(data: Vec<T>) -> Self {
        ...
        for i in 0..n {
            ...
            if min_elements.len() <= to {
                min_elements.push(w);
            } else if min_elements[to] > w {
                min_elements[to] = w;
            }
        }
        let mut to_left = Vec::with_capacity(n);
        let st = SparseTable::new(&min_elements[..], f);
        let mut mask = 0usize;
        for i in 0..n {
            if (i & AND_MASK) == 0 {
                mask = 0;
    ...
    pub fn query(&self, l: usize, r: usize) -> T {
        ...
        if bl == br {
            ...
            let mut best = min(self.data[idx_1].clone(), self.data[idx_2].clone());
            if bl + 1 < br {
                best = min(best, self.st.query(bl + 1, br - 1));
            }
            best
        }
    }
