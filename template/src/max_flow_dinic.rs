...
pub fn dinic<C: Number, E: FlowEdge<C>>(
...
fn dinic_push<'a, C: Number, E: FlowEdge<C>>(root: usize, mut flow: C, row: &mut Row<'a, C, E>) -> C {
    if root == row.sink {
        return flow;
    }
    let snapshot = flow;
    while row.iter[root] != usize::MAX && flow != C::ZERO {
        ...
        if row.dist[e.to()] + 1 == row.dist[root] && cap != C::ZERO {
            let sent = dinic_push(e.to(), unsafe_min(flow, cap), row);
            if sent != C::ZERO {
                flow = flow - sent;
