...
impl<S: Copy + Debug + Add<S> + Add<U, Output = S>, U: Copy + Debug + Eq + Add<U>> Debug for Segtree<S, U>
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Segtree").field("nodes", &self.nodes).field("L", &self.L).field("R", &self.R).field("zero_T", &self.nil_sum).field("zero_U", &self.nil_update).finish()
    }
}
impl<S: Copy + Debug + Add<S, Output = S> + Add<U, Output = S>, U: Copy + Debug + Eq + Add<U, Output = U>> Segtree<S, U>
{
    fn push_up(&mut self, root: usize) {
        self.nodes[root].sum = 
            self.nodes[root + 1].sum +
    ...
    fn initialize(&mut self, l: usize, r: usize, f: &impl Fn(usize) -> S) -> usize {
        ...
        self.nodes.push(NodeRow { right: usize::MAX, sum: self.nil_sum, update: self.nil_update });
        if l == r {
