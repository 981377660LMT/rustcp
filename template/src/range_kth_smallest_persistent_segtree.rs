...
/// ```
///
pub struct RangeKthSmallest<T: Ord + Clone + Debug> {
    st: PersistentSegtree<usize>,
    sorted: Vec<T>,
    cmp: Box<dyn Fn(&T, &T) -> Ordering>,
    roots: Vec<usize>,
}
impl<T: Ord + Clone + Debug> Debug for RangeKthSmallest<T> {
...
impl<T: Ord + Clone + Debug> RangeKthSmallest<T> {
    pub fn new(data: &Vec<T>, cmp: impl Fn(&T, &T) -> Ordering + 'static) -> Self {
        let mut sorted = data.clone();
        sorted.sort_by(|a, b| cmp(a, b));
        sorted.dedup_by(|a, b| cmp(a, b) == Ordering::Equal);
        let mut st = PersistentSegtree::new(0, sorted.len() - 1, 0, |a, b| a + b);
